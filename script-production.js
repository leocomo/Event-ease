...
async function handleCreateEvent(e) {
    ...
    try {
        ...
        // Schedule notification for users
...
async function rsvpToEvent(eventId) {
    try {
        ...
        showNotification('RSVP failed: ' + error.message, 'error');
    }
}
...
async function cancelRSVP(eventId) {
    try {
        ...
        showNotification('Cancel RSVP failed: ' + error.message, 'error');
    }
}

function isUserRSVPed(eventId) {
    const event = events.find(e => e.id === eventId);
    return event && event.rsvps.includes(currentUser.uid);
}

function loadUserRSVPs() {
    userRSVPsList.innerHTML = '';
    
    const userRSVPEvents = events.filter(event => 
        event.rsvps.includes(currentUser.uid) && new Date(event.date) > new Date()
    );
